...
#region Operations and Connection Methods
    ...
    internal protected void LoadLevelIfSynced()
    {
        ...
        if (sceneId is int)
        {
            if (PUNSceneManagerHelper.ActiveSceneBuildIndex != (int)sceneId)
                PhotonNetwork.LoadLevel((int)sceneId);
        }
        else if (sceneId is string)
        {
            if (PUNSceneManagerHelper.ActiveSceneName != (string)sceneId)
    ...
    protected internal void SetLevelInPropsIfSynced(object levelId)
    {
        ...
        if (PhotonNetwork.room.CustomProperties.ContainsKey(NetworkingPeer.CurrentSceneProperty))
        {
            ...
            if (levelIdInProps is int && PUNSceneManagerHelper.ActiveSceneBuildIndex == (int)levelIdInProps)
            {
                return;
            }
            if (levelIdInProps is string && PUNSceneManagerHelper.ActiveSceneName != null && PUNSceneManagerHelper.ActiveSceneName.Equals((string)levelIdInProps))
            {
                return;
            }
        }
